...
SELECT * FROM Departments INNER JOIN Departments ON Students.DepartmentId = Departments.DepartmentId;

SELECT s.StudentName, d.DepartmentName
FROM Students AS s
INNER JOIN Departments AS d ON s.DepartmentId = d.DepartmentId
ORDER BY s.StudentName DESC,d.DepartmentName ASC ;

SELECT avg(exam1) as `Exam1 Average`, min(exam2), max(exam3) FROM students;

SELECT exam1, count(exam1) as c  FROM students GROUP BY exam1 ORDER BY c DESC;
SELECT * FROM students GROUP BY exam1 ORDER BY c DESC;
